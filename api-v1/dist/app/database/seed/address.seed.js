...
const commune_model_1 = require("../../models/location/commune.model");
const village_model_1 = require("../../models/location/village.model");
class CitySeeder {
    static seed = async () => {
        try {
            await this.seedCity();
    ...
    static async seedCity() {
        try {
            await city_model_1.City.bulkCreate(cities);
        }
        catch (err) {
            console.log(err);
        }
    }
    static async seedDistrict() {
        try {
            await district_model_1.District.bulkCreate(districts);
        }
        catch (err) {
            console.log(err);
        }
    }
    static async seedCommune() {
        try {
            await commune_model_1.Commune.bulkCreate(communes);
        }
        catch (err) {
            console.log(err);
        }
    }
    static async seedVillage() {
        try {
            await village_model_1.Village.bulkCreate(villages);
        }
        catch (err) {
            console.log(err);
        }
...
//# sourceMappingURL=address.seed.js.map